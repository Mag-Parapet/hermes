...
pub fn generate_tokens(user_id: Uuid, name: String, email: String, role: UserRole, secret: &str) -> Result<(String, String), String> {
    let now = Utc::now();
    ...
    let access_claims = TokenClaims { sub: user_id.to_string(), name: name_str.clone(), email: email_str.clone(), role: role_str.clone(), exp: access_exp, iat };
    let refresh_claims = TokenClaims { sub: user_id.to_string(), name: name_str, email: email_str, role: role_str, exp: refresh_exp, iat };
